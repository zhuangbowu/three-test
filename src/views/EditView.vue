...
export default {
  ...
  methods: {
    init() {
      ...
      camera.position.set(0, 0, 1400); //设置相机位置
      ...
      point.position.set(-1200, 0, 0); //点光源位置
    ...
    meshRender() {
      ...
      let {size, slotSize, slotNum, arrayRule} = this.mesh;
      let meshSize = size.split(',');
      let parameters = {
        color: '#409EFF'
      }
      let mesh = this.addRectangle(meshSize[0], meshSize[1], meshSize[2], parameters);
      for (let i = 0; i < this.mesh.slotList.length; i++) {
        ...
        let sonMesh = this.addRectangle(sonMeshSize[0], sonMeshSize[1], sonMeshSize[2], parameters, mesh);
        sonMesh.position.set(-1, -((meshSize[1] - sonMeshSize[1]) / 2), -((meshSize[2] - (sonMeshSize[2] * (i + 1))) - ((i + 1) * 1.69)));
        if (this.mesh.slotList[i].children) {
          for (let j = 0; j < this.mesh.slotList[i].children.length; j++) {
            ...
            let sonMesh2 = this.addRectangle(sonMeshSize[0], ((sonMeshSize[1] - 9) / this.mesh.slotList[i].children.length), sonMeshSize[2] - 3, parameters, sonMesh);
            sonMesh2.position.set(-5, -(j * ((sonMeshSize[1] / this.mesh.slotList[i].children.length)) + 3), -1.5);
          }
        }
      }
